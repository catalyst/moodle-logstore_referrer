...
/**
 * Restore implementation for the (tool_log) logstore_referrer subplugin.
 *
...
class restore_logstore_referrer_subplugin extends restore_tool_log_logstore_subplugin {
    ...
    protected function define_logstore_subplugin_structure() {
        ...
        if (!in_array('logstore_referrer', $enabledlogstores)) {
        ...
        $elepath = $this->get_pathfor('/logstore_referrer_log');
    ...
    /**
     * Process logstore_referrer_log entries.
     *
     ...
     */
    public function process_logstore_referrer_log($data) {
        global $DB;

        $data = $this->process_log($data, get_config('logstore_referrer', 'jsonformat'));

        if ($data) {
            $DB->insert_record('logstore_referrer_log', $data);
        }
    }
}
