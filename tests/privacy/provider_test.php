 ...
 */
namespace logstore_referrer\privacy;
...
use logstore_referrer\privacy\provider;
...
class provider_test extends provider_testcase {
    ...
    public function test_get_contexts_for_userid() {
        ...
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $cm1ctx]);
        $e->trigger();
        ...
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $cm2ctx, 'relateduserid' => $u2->id]);
        $e->trigger();
        ...
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $c1ctx]);
        $e->trigger();
        ...
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $c2ctx, 'relateduserid' => $u3->id]);
        $e->trigger();
    ...
    public function test_add_userids_for_context() {
        ...
        $userlist = new \core_privacy\local\request\userlist($sysctx, 'logstore_referrer_log');
        ...
        $this->setUser($u1);
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $sysctx]);
        $e->trigger();
        // User two (userids) and three (relateduserid) should be added.
        $this->setUser($u2);
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $sysctx, 'relateduserid' => $u3->id]);
        $e->trigger();
    ...
    public function test_delete_data_for_user() {
        ...
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $c1ctx]);
        $e->trigger();
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $c2ctx]);
        $e->trigger();
        ...
        $this->assertTrue($DB->record_exists('logstore_referrer_log', ['userid' => $u1->id, 'contextid' => $c1ctx->id]));
        $this->assertEquals(3, $DB->count_records('logstore_referrer_log', ['userid' => $u1->id]));
        $this->assertEquals(2, $DB->count_records('logstore_referrer_log', ['userid' => $u2->id]));

        // Delete all the things!
        provider::delete_data_for_user(new approved_contextlist($u1, 'logstore_referrer', [$c1ctx->id]));
        $this->assertFalse($DB->record_exists('logstore_referrer_log', ['userid' => $u1->id, 'contextid' => $c1ctx->id]));
        $this->assertEquals(1, $DB->count_records('logstore_referrer_log', ['userid' => $u1->id]));
        $this->assertEquals(2, $DB->count_records('logstore_referrer_log', ['userid' => $u2->id]));
    }
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        // Confirm data present.
        $this->assertTrue($DB->record_exists('logstore_referrer_log', ['contextid' => $c1ctx->id]));
        $this->assertEquals(3, $DB->count_records('logstore_referrer_log', ['userid' => $u1->id]));
        $this->assertEquals(2, $DB->count_records('logstore_referrer_log', ['userid' => $u2->id]));

        // Delete all the things!
        provider::delete_data_for_all_users_in_context($c1ctx);
        $this->assertFalse($DB->record_exists('logstore_referrer_log', ['contextid' => $c1ctx->id]));
        $this->assertEquals(1, $DB->count_records('logstore_referrer_log', ['userid' => $u1->id]));
        $this->assertEquals(1, $DB->count_records('logstore_referrer_log', ['userid' => $u2->id]));
    }
    ...
    public function test_delete_data_for_userlist() {
        ...
        $this->assertEquals(4, $DB->count_records('logstore_referrer_log'));

        $userlist = new \core_privacy\local\request\approved_userlist($sysctx, 'logstore_referrer_log', [$u1->id, $u3->id]);
        provider::delete_data_for_userlist($userlist);
        // We should have a record for u2 and u4.
        $this->assertEquals(2, $DB->count_records('logstore_referrer_log'));

        $records = $DB->get_records('logstore_referrer_log', ['contextid' => $sysctx->id]);
    ...
    public function test_export_data_for_user() {
        ...
        $path = [get_string('privacy:path:logs', 'tool_log'), get_string('pluginname', 'logstore_referrer')];
        $this->enable_logging();
        $manager = get_log_manager(true);

        // User 1 is the author.
        $this->setUser($u1);
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $c1ctx, 'other' => ['i' => 0]]);
        $e->trigger();

        // User 2 is related.
        $this->setUser(0);
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $c1ctx, 'relateduserid' => $u2->id,
            'other' => ['i' => 1]]);
        $e->trigger();
        ...
        $this->setAdminUser();
        \core\session\manager::loginas($u3->id, $sysctx);
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $c1ctx, 'relateduserid' => $u4->id,
            'other' => ['i' => 2]]);
        $e->trigger();

        // Confirm data present for u1.
        provider::export_user_data(new approved_contextlist($u1, 'logstore_referrer', [$c2ctx->id, $c1ctx->id]));
        ...
        writer::reset();
        provider::export_user_data(new approved_contextlist($u2, 'logstore_referrer', [$c2ctx->id, $c1ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertEmpty($data);
        $data = writer::with_context($c1ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        ...
        writer::reset();
        provider::export_user_data(new approved_contextlist($u3, 'logstore_referrer', [$c2ctx->id, $c1ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertEmpty($data);
        $data = writer::with_context($c1ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        ...
        writer::reset();
        provider::export_user_data(new approved_contextlist($u4, 'logstore_referrer', [$c2ctx->id, $c1ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertEmpty($data);
        $data = writer::with_context($c1ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        ...
        $this->setUser($u1);
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $c2ctx, 'relateduserid' => $u2->id,
            'anonymous' => true]);
        $e->trigger();
        $this->setAdminUser();
        \core\session\manager::loginas($u3->id, $sysctx);
        $e = \logstore_referrer\event\unittest_executed::create(['context' => $c2ctx, 'relateduserid' => $u4->id,
            'anonymous' => true]);
        $e->trigger();

        // Confirm data present for u1.
        provider::export_user_data(new approved_contextlist($u1, 'logstore_referrer', [$c2ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        ...
        // Confirm data present for u2.
        writer::reset();
        provider::export_user_data(new approved_contextlist($u2, 'logstore_referrer', [$c2ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        $this->assertEquals(transform::yesno(true), $data->logs[0]['action_was_done_anonymously']);
        ...
        // Confirm data present for u3.
        writer::reset();
        provider::export_user_data(new approved_contextlist($u3, 'logstore_referrer', [$c2ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        $this->assertEquals(transform::yesno(true), $data->logs[0]['action_was_done_anonymously']);
        $this->assertEquals(transform::yesno(true), $data->logs[0]['author_of_the_action_was_you']);
        ...
        // Confirm data present for u4.
        writer::reset();
        provider::export_user_data(new approved_contextlist($u4, 'logstore_referrer', [$c2ctx->id]));
        $data = writer::with_context($c2ctx)->get_data($path);
        $this->assertCount(1, $data->logs);
        $this->assertEquals(transform::yesno(true), $data->logs[0]['action_was_done_anonymously']);
        $this->assertArrayNotHasKey('author_of_the_action_was_you', $data->logs[0]);
    ...
    protected function enable_logging() {
        set_config('enabled_stores', 'logstore_referrer', 'tool_log');
        set_config('buffersize', 0, 'logstore_referrer');
        set_config('logguests', 1, 'logstore_referrer');
    }
