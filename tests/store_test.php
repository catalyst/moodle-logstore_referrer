...
class store_test extends \advanced_testcase {
    ...
    public function test_log_writing(bool $jsonformat) {
        ...
        set_config('jsonformat', $jsonformat ? 1 : 0, 'logstore_referrer');
        ...
        $this->assertEquals(array('logstore_referrer'), array_keys($stores));
        /** @var \logstore_referrer\log\store $store */
        $store = $stores['logstore_referrer'];
        $this->assertInstanceOf('logstore_referrer\log\store', $store);
        $this->assertInstanceOf('tool_log\log\writer', $store);
        $this->assertTrue($store->is_logging());

        $logs = $DB->get_records('logstore_referrer_log', array(), 'id ASC');
        $this->assertCount(0, $logs);

        $this->setCurrentTimeStart();

        $this->setUser(0);
        $event1 = \logstore_referrer\event\unittest_executed::create(
        ...
        \logstore_referrer_restore::hack_executing(1);
        $event2 = \logstore_referrer\event\unittest_executed::create(
            array('context' => \context_module::instance($module2->cmid), 'other' => array('sample' => 6, 'xx' => 9)));
        $event2->trigger();
        \logstore_referrer_restore::hack_executing(0);
        ...
        // Test buffering.
        set_config('buffersize', 3, 'logstore_referrer');
        $manager = get_log_manager(true);
        $stores = $manager->get_readers();
        /** @var \logstore_referrer\log\store $store */
        $store = $stores['logstore_referrer'];
        $DB->delete_records('logstore_referrer_log');

        \logstore_referrer\event\unittest_executed::create(
            array('context' => \context_module::instance($module1->cmid), 'other' => array('sample' => 5, 'xx' => 10)))->trigger();
        $this->assertEquals(0, $DB->count_records('logstore_referrer_log'));
        \logstore_referrer\event\unittest_executed::create(
            array('context' => \context_module::instance($module1->cmid), 'other' => array('sample' => 5, 'xx' => 10)))->trigger();
        $this->assertEquals(0, $DB->count_records('logstore_referrer_log'));
        $store->flush();
        ...
        \logstore_referrer\event\unittest_executed::create(
            array('context' => \context_module::instance($module1->cmid), 'other' => array('sample' => 5, 'xx' => 10)))->trigger();
        $this->assertEquals(5, $DB->count_records('logstore_referrer_log'));
        \logstore_referrer\event\unittest_executed::create(
            array('context' => \context_module::instance($module1->cmid), 'other' => array('sample' => 5, 'xx' => 10)))->trigger();
        $this->assertEquals(5, $DB->count_records('logstore_referrer_log'));
        \logstore_referrer\event\unittest_executed::create(
        ...
        \logstore_referrer\event\unittest_executed::create(
            array('context' => \context_module::instance($module1->cmid), 'other' => array('sample' => 5, 'xx' => 10)))->trigger();
        $this->assertEquals(8, $DB->count_records('logstore_referrer_log'));

        // Test guest logging setting.
        set_config('logguests', 0, 'logstore_referrer');
        set_config('buffersize', 0, 'logstore_referrer');
        ...
        \logstore_referrer\event\unittest_executed::create(
            array('context' => \context_module::instance($module1->cmid), 'other' => array('sample' => 5, 'xx' => 10)))->trigger();
        $this->assertEquals(1, $DB->count_records('logstore_referrer_log'));
    ...
    public function test_get_supported_reports() {
        ...
        $supportedreports = $logmanager->get_supported_reports('logstore_referrer');
    ...
    /**
     * Test that the referrer log cleanup works correctly.
     */
    public function test_cleanup_task() {
        ...
        $DB->insert_record('logstore_referrer_log', $record);
        ...
        set_config('loglifetime', 1, 'logstore_referrer');

        $this->expectOutputString(" Deleted old log records from referrer store.\n");
        $clean = new \logstore_referrer\task\cleanup_task();
        $clean->execute();
    ...
    public function test_decode_other($value) {
        $this->assertEquals($value, \logstore_referrer\log\store::decode_other(serialize($value)));
        $this->assertEquals($value, \logstore_referrer\log\store::decode_other(json_encode($value)));
    }

    public function test_decode_other_with_wrongly_encoded_contents() {
        $this->assertSame(null, \logstore_referrer\log\store::decode_other(null));
    }
    ...
    public function test_backup_restore(bool $jsonformat) {
        ...
        $records = array_values($DB->get_records('logstore_referrer_log',
        ...
        set_config('jsonformat', $jsonformat ? 0 : 1, 'logstore_referrer');
